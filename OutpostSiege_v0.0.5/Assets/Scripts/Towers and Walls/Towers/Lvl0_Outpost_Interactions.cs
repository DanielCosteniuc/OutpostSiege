...
public class Lvl0_Outpost_Interaction : MonoBehaviour
{
    ...
    private TowerWalls_Generation outpostGenerator; // trebuie trimisă în prefab/outpost
    ...
    private void Start()
    {
        ...
        outpostGenerator = FindObjectOfType<TowerWalls_Generation>(); // presupunem că e unul în scenă
    }
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        ...
        {
            player.ReturnCoinsToPlayer(coinsInserted); // returnăm banii

            foreach (var holder in coinInstances)
            {
                Destroy(holder);
            }
